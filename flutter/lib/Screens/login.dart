...
import 'package:smartcanteen/Screens/user/forgotPassword.dart';
import 'package:smartcanteen/Screens/widgets/buildTextfield.dart';
import 'package:smartcanteen/Services/loginApi.dart';
...
class _LoginScreenState extends State<LoginScreen> {
                ...
                child: Column(
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        TextButton(onPressed: () { Navigator.of(context).push(MaterialPageRoute(builder: (context) => ForgotPasswordScreen(),));  }, child: Text("Forgot password?"),)
                      ],
                    ),
                    ...
                    ValueListenableBuilder(
                      ...
                      builder: (context, value, child) {
                        ...
                        child: ElevatedButton(
                          ...
                          child: isLoading.value? CircularProgressIndicator(): Text("Login",
                              style:
  ...
  LoginCheck(BuildContext context) async {
    ...
    if (checkStatus == null) {
      ...
      isLoading.value = false;
      if (checkStatus.userType == "canteen") {
        await saveLoginData("canteen", checkStatus.access, checkStatus.userId,checkStatus.userType,checkStatus.email,checkStatus.username);
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => canteenHomeScreen()),
        );
      } else if (checkStatus.userType == "staff") {
        isLoading.value = false;
        await saveLoginData("staff", checkStatus.access, checkStatus.userId,checkStatus.userType,checkStatus.email,checkStatus.username);
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => HomeScreen(userType: checkStatus.userType,)),
        );
      } else if (checkStatus.userType == "student") {
        isLoading.value = false;
        await saveLoginData("student", checkStatus.access, checkStatus.userId,checkStatus.userType,checkStatus.email,checkStatus.username);
        userDetails.add(checkStatus);
        print("token :  ${userDetails[0].access}");
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => HomeScreen(userType: checkStatus.userType,)),
        );
  ...
  saveLoginData(String type, String token, int userid,String userType, String mail, String username) async {
    print("here shared");
    final shared_pref = await SharedPreferences.getInstance();
    shared_pref.setString("loginData", type);
    shared_pref.setString("token", token);
    shared_pref.setInt("userId", userid);
    shared_pref.setString("userType", userType);
    shared_pref.setString("userName", username);
    shared_pref.setString("userMail", mail);
    print("done shared");
  }
}
